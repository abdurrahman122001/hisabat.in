...
if(isset($_POST['submit'])){
    ...
    $res2 = mysqli_query($con, "SELECT id,email,password_hash,role,status,`password` FROM users WHERE email='$emailEsc' LIMIT 1");
    if ($res2 === false) {
    ...
    if (!$u) {
        ...
        if (!$isActive) {
            ...
            $legacyPass = isset($u['password']) ? (string)$u['password'] : '';
...
<html lang="az">
  ...
  <style>
    ...
    body{margin:0;background:radial-gradient(1200px 600px at 20% 0%, rgba(124,58,237,.35), transparent 55%), radial-gradient(900px 520px at 90% 20%, rgba(253,183,20,.25), transparent 55%), linear-gradient(180deg, #0b1220 0%, #0f172a 100%);}
    ...
    .login-input{width:100%;border-radius:14px;border:1px solid var(--gri-border);padding:12px 14px;font-weight:700;color:var(--gri-text);background:#fff;outline:none;transition:all .15s ease;}
    ...
    .login-btn{width:100%;border-radius:14px;padding:12px 14px;border:none;background:linear-gradient(90deg, var(--gri-accent) 0%, #ffd56a 60%, var(--gri-accent) 100%);color:#111827;font-weight:900;letter-spacing:.3px;box-shadow:0 14px 30px rgba(253,183,20,.24);cursor:pointer;transition:transform .12s ease, box-shadow .12s ease;}
          ...
          <div class="login-right">
            ...
            <form class="login-form" action="login.php" method="post">
              <div class="login-field">
                ...
                <input type="email" class="login-input" id="exampleInputEmail1" name="email" placeholder="məs: admin@example.com" required>
              </div>
